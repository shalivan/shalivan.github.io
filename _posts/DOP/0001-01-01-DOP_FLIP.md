...
<span style="color: #F09900"> HIP:</span> [Basic flips](/src/hip/DOP_FLIP.hiplc)
...
`Particle` method only works when a collision is represented by an actual DOP object.  
...
#### Separation:  
Additional separation that prevent volume compression verriten by velocity.

#### Droplets:  
Tend to clamp toghether when partic drop below certain density turn to droplets more likely. After blend back to fluid: `Blend` / `Merge` / `Kill`
...
## Volume Motion:  
Kernel `Swirly` -  is not as noisy and turb. and keep vorticity better  
smoothing. Implemented with the newer APIC method.  
`Splashy`  
...
||lo|high|
 |-|-|-|
 Viscosity |1 - Water |5-10M - Silicon|
 Slip on collision  | 0 -  collider v. (Can't slip - Sticky)  | 1 - fluid v. can slip - slide fluid [opposite to: stick to collision]
...
||-|0|+|
|-|-|-|-|
Divergence |Clump them together | Incompressive fluid have not "scale factor" (have: swirl , shear and trans)| Cause particles to spread out



#### Surface Tension:     
Tendency of liquid surfaces to shrink into the minimum surface area possible.  *Require small scale but, In small scale it can be unstable > substeps*  

||lo|high|
|-|-|-|
Surface Tension | 48 - more motion | 200 - 48000 - less motion, prevent fall,  tend to min. surface area more
