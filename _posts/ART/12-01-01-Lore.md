...
The backstory created around a fictional universe. Believable and convincing settings. SHould iintertwine with desing and gameplay [zalozenia]. **Holon** represents a way to overcome the dichotomy between parts and wholes, Koestler's Holon Simultaneously Whole & Part.
...
- Human world `Self`/`Individual`, `Family`, `Community`, `Others` (City), `World`, `Cosmos`  
- Levels of Organization of the Human Organism: `Organism` > `Organs` > `Tissues` > `Vells` > `Organelles` > `Molecules` > `Atoms`


#### Peaces scales
Stuart Brand  and (architect Frank Duffy)

Six layers that function simultaneously at different speeds within society
...
- primitive
- steam - with physical machinery / steam water / temp / kinetic
- electronic - with wires / solar
- future - magic
...
- Human:  `Self` (culture, history)> `body` (structure terrain) > `Mind` (Governance > economic) > `Fuel` (Energy> Resources)
...
### Societies types
Agriculture vs Hunter Gatherers  societies
