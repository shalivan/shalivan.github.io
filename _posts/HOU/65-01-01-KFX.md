...
- **Joint** - Point that is part of hierarchy (defined by hierarchy). In world space
  - `@name` unique per point + As topology change rest of points stays where supposed to  
  - `@P`  (3flt)    - handle world space translation
  - `@transform` (9flt - matrix) - (matrix3) translation, rotation, scale ( `joints orientation` )
  - `@resttransform` - can add 4x4
- **Controls**
  - `@name` on prims  
