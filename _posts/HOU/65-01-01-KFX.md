...
You can drag points to get vectors from viewport. (in `Get Point Transform` VOP) and sav to `Set Point Transform` - Export data from VOP   



constraints in vop: copy world transf
get point transform  (cand set input as source), and should show point  >>
set point tansform.. >

#### World position copy
`xfrom` - global - work with `i@scaleinheritance`


#### Local position
need at least 2 points

`local xform` - scale not effecting   
`effective xform` - depending on values inheritance attributes.  - `i@scaleinheritance` - how much depand on parent and scale inherent attribute.
