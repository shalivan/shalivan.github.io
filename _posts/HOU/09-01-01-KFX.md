...
> Pxlink: [CFX](/cfx/), [Unreal Animation](/uanimation/), [Animation](/animation/)   
>Obsidian:  [[10-01-01-Animation]] [[08-01-01-CFX]] [[05-01-01-U_Animation]]
...
- **Joint** - Point that is part of hierarchy (defined by hierarchy). In world space
  ...
  - `3@transform` (9flt - matrix) - (matrix3) rotation, scale ( `joints orientation` )
  - `v@P` - position
...
Skeleton components: 
- **Hierarchy** - determined by vertex order (not point order). (reverse SOP will change direction).  
- **Transforms** 
	- **Worlds space**  `@transform` (9flt) input world transform into SOPs is ground truth, - Used for rigging and copying geo.  Rotation and scale only. Position from @P. 
	- **Local space**  `@localtransform` (16flt) are calculated per operation in some of kinefx nodes.  - Used for animation.  (Thanks for that is easy to work with other inputs). ( computetransforms recalculate local transform).  Position rot and scale. 
	- `@_efectivelocaltransform` (16flt) - work with  `scale inheretance`
...
- **Names** 
	- string on points. Should be unique.


## Geometry to skeleton

![[createskeleton2.png]]

**Name** 
  - Name SOP 
  - Enumerate SOP (on points, string)
**Orientation**
...
**Auto Setup**
-  [ Skeleton SOP  ] - Click stache input to freez (like cache). you can disconnect.  not for procedural update 
...
##### Tips
 ...
 We don't need to connect geometry. Points are enough 
...
##### [ Skeleton] SOP
Create or Modify. Can control bone placement, rename bones, orient selected bones. It create `@localtransform`.  (with correct orientation)
...
culr like, for each point transform. 
VEX
`rotate(4@localtransform, radians(10), {1,0,0})`
VOP
![[Pasted image 20240604221612.png]]
