...
![](/src/noise/sprs.jpg)
```
v@noise = fit(snoise(@P*chv('freq')-chv('offset'), chi('turb'), ch('rough'), ch('atten')),-1,1,0,1)*ch("amp");
```
...
onoise similar to wnoise and vnoise. However, they are marginally less efficient in computation and don’t have the same characteristics.
 ```
v@noise = fit(onoise(@P*chv('freq')-chv('offset'), chi('turb'), ch('rough'), ch('atten')),-1,1,0,1)*ch("amp");
```
...
![](/src/noise/ali.jpg)
```
v@noise = anoise(@P*freq - offset, turb, rough, atten) * amp;
```
...
#### Curl
>Divergence free noise fileld is based on the cross product of the derivatives of two simplex noise functions. (vector operator that produces a scalar field)
```
...
`![](/Sources/noise/HoudiniNoise.jpg)`
