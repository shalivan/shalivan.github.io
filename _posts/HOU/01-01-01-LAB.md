...
### Rotation texture > Position alpha
- `Export Compressed Normals in Position Alpha` - Compresses vector to float. Stores them in `Position Texture's A` channel. Lossy compression. U: Check export compressed normals in position A & use compressed normals
