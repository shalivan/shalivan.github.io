...
- **Primitive** & **Attributes** are primary container objects Every object is primitive
- **Namespace** - path to obj in stage. Important!
- **Layer** - file on disk
- **model** - All Assets in USD
...
## Kind
A method of defining hierarchy oin primitives (like tag) help define role in hierarchy. In most pipelines should be automated. authoring: (like in sop import / configure primitive).  Can help in selection. (can change behavior's type in selection RMB menu)
...
### Component
Cannot contain froups or asemlies! Only able to contain Subcomponents. (most of time when import sops or smaller assets.)
